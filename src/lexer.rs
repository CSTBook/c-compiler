...
pub fn lexer(filename: String) -> Vec<String>{
    ...
    let token_regexes = vec![r"int\b", r"void\b", r"return\b", r"\(", r"\)", r"\{", r"\}", r";",r"[a-zA-Z_]\w*\b", r"[0-9]+\b",r"~",r"-",r"--"];
    ...
    while !contents.is_empty() {
        ...
        if valid_tokens.len()==0 && !contents.is_empty() {panic!("invalid token found in {}", contents);}
        if valid_tokens.len()==0 {break;}

        let mut longest_token=valid_tokens.get(0).unwrap().to_string();
        ...
        contents = String::from(&contents[contents.find(&longest_token).unwrap()+&longest_token.len()..]);
